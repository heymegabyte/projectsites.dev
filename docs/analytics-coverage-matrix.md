...
- **Top EXIT (last) pages (✅ DONE end-to-end 2026-09-25):** the complement to entry pages — WHERE visits END
  ...
  disclaimer. (At the time this was **deliberately isolated from the `session_id` COLUMN** — then per-pageload
  + feeding uniqueSessions/bounce; that column is now ITSELF per-tab as of the 2026-09-25 Visits correctness fix,
  so uniqueSessions/bounce/funnel finally count real sessions too.) +5 Jest (fold/skip-null · window-fn query shape · fail-soft ·
...
- **CF GraphQL `httpRequestsAdaptiveGroups`** (`services/multi_url_analytics.ts`) is **fallback-only**
  ...
  - **Ranked backlog (updated 2026-09-25):** (1) ✅ **top-pages VISITS — DONE**; (2) ✅ **UTM medium
    ...
    summary wires byUtmMedium) + card/CSV Karma. (3) ✅ **delivery `by_cache_visits` — DONE this fire**:
    the `cache` httpRequestsAdaptiveGroups sub-query now also selects `sum{visits}` (same per-host
    request, no new CF call), so each cache-state carries REAL VISITORS, not just request count/bytes.
    `HostDelivery.by_cache_visits` + merge + `buildDeliverySummary(byCacheVisits)` → `DeliverySummary.cache.{hit,miss,uncacheable}_visits`,
    mirroring the existing `by_status_visits`/`by_cache_bytes` pattern exactly. `DeliveryCardComponent` now
  ...
  - **REJECTED (2026-09-25 scan) — byConversionSection (❌ FALSE GAP, fully built):** an agent flagged the
    beacon's `section` field (conversion CTA container) as "captured but never aggregated." It IS captured
    (app.js:214) + persisted (analytics.ts:309), AND fully surfaced: `getConversionsBySection` (AN27,
    `site_analytics/service.ts:212`) + route `GET /api/sites/:siteId/analytics/sections` + a test + a
    self-fetching `SectionAttributionComponent` wired into `analytics-dashboard.component.ts:100`. A narrow
    grep on `analytics.component.ts` misses it (separate self-fetching component in the sibling dashboard).
    Also rejected: **concierge_open/message mirror** (concierge is Gallery-only, dead on real sites — no
    owner value); **coloCode / edge device-type** breakdowns (available but low customer value / redundant
    with the first-party device breakdown). Lesson: agents over-report; a "captured but not aggregated"
    claim needs a trace of BOTH the aggregator files AND the self-fetching cards (not just getTrafficSummary
    + the main dashboard).
...
| Visits (browsing sessions) | D1 visitor_events, `COUNT(DISTINCT session_id)`; `session_id` = the app.js beacon's per-tab-session id (the ingest stores `event.sessionId` verbatim — `routes/analytics.ts:160`) | none | site_id | D1 | none | ✅ live — **CORRECTNESS FIX (2026-09-25):** the beacon's `SESSION_ID` was regenerated per *pageload* (an earlier version of THIS row wrongly claimed a server `SHA-256(ip\|ua\|day)` hash — no such hash exists; verified against source), so `COUNT(DISTINCT session_id)` ≈ **pageviews**, not sessions. Now `SESSION_ID` is sourced from the persisted per-tab `ps_sess` UUID (fallback per-pageload only when sessionStorage is unavailable), so a 5-page visit is ONE session → **Visits + bounce + funnel all count real sessions**. KPI tile reads **"Visits"**; glossary rewritten from "visitor-days" to "browsing sessions (one tab visit)". Corrects going forward as fresh traffic accrues (older rows keep their per-pageload ids — mixed history, honest). Earlier: public share report reads `uniqueSessions` (not a nonexistent `uniqueVisitors` key). | Visits KPI tile · bounce · funnel · public share report |
