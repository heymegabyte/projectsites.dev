...
- **RUM (Cloudflare Web Analytics)** — **INTEGRATED AS A SERVER SOURCE (2026-09-25, this fire).**
  ...
  - **Next (UI):** a `CloudflareRumCardComponent` in `/admin/analytics` labeled "Cloudflare RUM
    (independent measurement)", shown beside the first-party CWV card. Deferred this fire only to
    avoid colliding with the concurrent drilldown-filter session editing `analytics.component.ts`.
