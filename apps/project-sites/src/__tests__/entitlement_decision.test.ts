 ...
 */
import { decideEntitlement, isEntitled, ENTITLING_STATUSES } from '../services/entitlement_decision.js';
...
describe('entitlement_decision: status gate', () => {
  it('entitles active and trialing (trialing counts)', () => {
    ...
    expect(decideEntitlement({ status: 'trialing' })).toEqual({ entitled: true, reason: 'entitled' });
    expect([...ENTITLING_STATUSES]).toEqual(['active', 'trialing']);
  });
  it('denies past_due / canceled / unpaid / paused / null / unknown', () => {
    for (const s of ['past_due', 'canceled', 'unpaid', 'paused', 'incomplete', 'some_future_status']) {
...
describe('entitlement_decision: plan floor (orthogonal to status)', () => {
  it('requires the plan to meet a declared floor even with an entitling status', () => {
    expect(decideEntitlement({ status: 'active', plan: 'starter', featurePlanFloor: 'pro', planRank: RANK })).toEqual({
      entitled: false,
      reason: 'below_plan_floor',
    });
    expect(decideEntitlement({ status: 'active', plan: 'pro', featurePlanFloor: 'pro', planRank: RANK }).entitled).toBe(true);
    expect(decideEntitlement({ status: 'active', plan: 'business', featurePlanFloor: 'pro', planRank: RANK }).entitled).toBe(true);
  });
  ...
  it('fail-closed when a floor is declared but the plan is unknown / no rank map', () => {
    expect(decideEntitlement({ status: 'active', plan: 'mystery', featurePlanFloor: 'pro', planRank: RANK }).entitled).toBe(false);
    expect(decideEntitlement({ status: 'active', plan: 'pro', featurePlanFloor: 'pro' }).entitled).toBe(false);
  });
});
