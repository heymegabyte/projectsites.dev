...
export const FLAG_DOCS: Record<string, FlagDocs> = {
  ...
  live_build_stream: {
    checklist: [
      ...
      'Noise-filtered twice: isBuildLogNoise drops Claude Code control-plane + provider-transport (402 / unrecognized_model) lines at ingest AND render — a genuine build error still shows (red)',
    ...
    explanation:
      'STREAMING BUILD THEATER — turns the /waiting page into a live, trust-building view of the AI actually building the site. The build container (container-server.mjs) captures the Claude Code process stdout/stderr, buffers complete lines, and POSTs throttled, bounded, secret-redacted batches to the worker’s HMAC-signed ingest (POST /api/internal/build-log). The worker (gated by THIS flag, fail-closed to 404) redacts, drops Claude Code control-plane + provider-transport noise (isBuildLogNoise — so a dead build-LLM balance can never spam the owner with 402 / model-catalog errors), and caps each batch (build_log.ts), writing one audit_logs row per real line (action claude.output); the existing /waiting terminal poll (getSiteLogs → toBuildLogLine) renders them with tasteful coloring (green success / cyan phase / red error) + a heartbeat so a long build never looks frozen. Bounded end-to-end so a runaway build can neither flood audit_logs nor break the build (every stream fault is fail-soft). OFF is a clean no-op: the container’s POSTs 404 and the terminal shows only the coarse phase-step lines.',
    smoke_test: [
