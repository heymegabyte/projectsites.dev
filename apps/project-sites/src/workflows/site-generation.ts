...
export function buildPrompt(params: SiteGenerationParams): string {
  ...
  return [
    ...
    "   - Content DENSITY (the homepage is the conversion + SEO surface — thin loses to the source): the homepage MUST carry >=800 words of REAL, substantive business content (aim 900-1100), and every sub-page >=400. Deployed sites keep shipping ~600-word homepages (one-line sections) that LOSE the beat-the-source density dimension — do NOT. Give every section 2-4 real sentences grounded in _research.json/_scraped_content.json, never a single-line stub: expand About into a real story (history/mission/what-makes-them-different), write a genuine 2-3 sentence description under EACH service/offering (not just a name), add a Why-Us / trust section, and add a real FAQ (3-5 Q&A the business actually gets) + a local-context paragraph (neighborhood/city they serve). Depth comes from the business's ACTUAL story, services, and reviews — never filler, never repetition, never banned slop. A section with only a heading + one line is a quality miss.",
