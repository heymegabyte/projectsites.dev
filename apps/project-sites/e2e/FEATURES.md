...
| Admin CWV — cold-load LCP **AND CLS** gate per `/admin` route (9 routes) | `measure-admin-cwv.mjs` | admin-verify | GREEN* | AL-850 — the probe verdict/exit gated LCP ONLY, silently passing 0.20-0.34 CLS breaches (lying-green). Hardened: **CLS≤0.10 ceiling (fails) / 0.05 target (⚠️)** alongside LCP; default routes extended to the heavy lazy-chunk sections (snapshots/docs/social) the old set never measured; wired into `run-all.mjs`. Prod 8/9 ✓ (all LCP <300ms). **docs root-fixed 0.0693→0.0094** (loading/error/explorer mutually exclusive → explorer materializes in the loading card's place). *Tracked breach: `/admin/snapshots` CLS 0.34 — its 3 insight panels (readiness/sparkline/timeline) are flag+data-gated so a static reserve regresses flag-off orgs; needs a flag-aware parent reserve (next fire). New tool: `_cls-attribution.mjs` (per-element shift attribution). |
...
| Platform marketing homepage — interactive sections (§ D) | `marketing-sections.spec.ts` | loop | GREEN | AL-826 — **REWRITTEN** from the DELETED vanilla homepage (all 25 old tests prod-RED; only false-green vs stale `sites-staging.megabyte.space`). Homepage-first Angular journey, 8 tests / **8 GREEN on prod (16.1s)**: all sections render (hero/social-proof/how-it-works/features/pricing/faq/footer) · FAQ accordion expand+collapse (click + keyboard Enter, `aria-expanded` toggles) · honest AL-581 counters (≥3 `app-rolling-counter`, never fabricated 2480) · pricing has real plan copy · hero-CTA scrolls to section (SPA sentinel = no reload) + 0 console · footer nav links · **homepage axe 0-critical ×6bp**. Enrolled in the prod suite. |
| Build-progress /waiting flow + graceful load-error (post-create) | `waiting-and-terminal.spec.ts` | loop | GREEN | AL-827 — **REWRITTEN** from the DELETED vanilla waiting screen (13 old tests prod-RED). **Root-fixed a real break:** the auth-gated build-status poll died on the first 401 (switchMap propagated the error → stream stopped), leaving an owner with an expired session / a shared `/waiting?id=` link staring at a **perpetual FAKE "Building…" overlay** forever. Fix: `catchError→of(null)` keeps the poll alive; after 2 never-loaded ticks it degrades to a graceful "sign in to continue" card (`shouldDegradeToLoadError`, +2 Karma) + a structured `waiting_poll_load_failed` log. 4 prod tests / **4 GREEN (21.1s)**: no-id→home · unauth-id→graceful card (not overlay), 0 uncaught errors · sign-in→`/signin?returnUrl=/waiting` · go-home→home. Enrolled in the prod suite. |
| Platform SPA routing — footer legal-link SPA-nav + soft-404 doctrine | `spa-routing.spec.ts` | loop | GREEN | AL-830 — **REWRITTEN** from the DELETED vanilla homepage (`#screen-search`/`.footer-bottom`/"Astro"; all prod-RED, false-green vs stale staging). Homepage-first, 8 prod tests / **8 GREEN (8.7s)**: 3 footer legal links (privacy/terms/content) SPA-navigate (no full reload, window sentinel) + render real H1 · 3 direct-URL routes 200 + real H1 · **a bogus path returns a REAL 404 status** (the soft-404 gate the old `status<500` check was blind to) · a real route still 200s. 0 console errors. Enrolled in the prod suite. |
| **TOTAL** | **24 specs** | convergence | **167+ tests GREEN** | Passes 1-29 + AL-804/826/827/830 |
