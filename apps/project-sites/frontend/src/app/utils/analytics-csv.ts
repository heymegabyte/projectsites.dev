 ...
 *  - it includes the D1 breakdowns (device / browser / OS / channel / conversions /
 *    Core Web Vitals) that work for EVERY site, not just the CF-zone envelope. A CWV /
 *    bounce value is emitted only when it was actually measured (never a fabricated 0).
 */
...
export function buildAnalyticsCsv(input: AnalyticsCsvInput): string {
  ...
  // D1 breakdowns — the universal (all-site) data the prior export dropped. The
  // device/browser/os trio mirrors the "Devices & platforms" card, one row per label.
