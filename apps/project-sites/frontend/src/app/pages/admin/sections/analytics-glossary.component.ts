...
const DEFS: readonly MetricDef[] = [
  ...
  {
    def: 'Browsing sessions — one visit from a single browser tab, grouped cookielessly (the group is cleared when the tab closes). A visit that views five pages counts once, so this is NOT the same as page views. Someone returning later, or on another device, counts as a new visit, so it is also MORE than the number of unique people. Over long ranges a visit that spans multiple days is counted on each day.',
    source: 'ProjectSites (first-party)',
    term: 'Visits',
  },
  {
...
@Component({
  ...
  styles: [
    `
      ...
      .ag-src[data-src='Real-user (browser)'] {
  ...
  template: `
    <details class="ag" data-testid="an-glossary">
      ...
      <p class="ag-note">
        ...
        &ldquo;First-party&rdquo; means ProjectSites measured it on your site; &ldquo;Cloudflare
        edge&rdquo; is sampled request data available for custom domains. Real-user speed metrics
        come from the ProjectSites beacon on every page — we don’t rely on the Cloudflare Web
        Analytics beacon, so they’re collected on every domain with no extra setup.
      </p>
    </details>
  `,
})
