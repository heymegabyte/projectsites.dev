 ...
 *
 * Renders three pageview breakdowns (traffic source + medium + campaign) from
 * `traffic.byUtmSource` / `byUtmMedium` / `byUtmCampaign` — the `utm_source` / `utm_medium` /
 * `utm_campaign` params on TAGGED visits (an ad, email, or social link the owner tagged).
 * `utm_medium` is the RAW owner-set medium (cpc/email/newsletter), distinct from the coarse
 * derived `byChannel`. Focused, standalone,
    ...
    <section class="card cmp" data-testid="an-campaigns">
      ...
      <h3 class="cmp-head">
        ...
        <span
          ...
          title="Pageviews from links you tagged with utm_source / utm_medium / utm_campaign (ads, email, social). Untagged direct/organic traffic is not a campaign and is excluded. A count, never an estimate."
          >last {{ windowDays() }} {{ windowDays() === 1 ? 'day' : 'days' }} · tagged visits
          only</span
        >
      </h3>
      ...
      @if (hasAny()) {
        ...
        <p class="cmp-empty" data-testid="an-campaigns-empty">
          ...
          <code>utm_source</code>, <code>utm_medium</code> and <code>utm_campaign</code> to your
          links — e.g.
          <code>?utm_source=instagram&amp;utm_medium=social&amp;utm_campaign=spring-sale</code>.
          Tagged visits then appear here as a real breakdown, never an estimate.
        </p>
      }
    </section>
  `,
...
export class CampaignBreakdownComponent {
  ...
  /** `traffic.byUtmMedium` — tagged pageviews by utm_medium (cpc / email / newsletter / …). */
  readonly mediums = input<CampaignCount[]>([]);
  ...
  readonly groups = computed<CampaignGroup[]>(() =>
    (
      [
        ...
        { key: 'medium', label: 'Mediums', rows: this.top(this.mediums()) },
