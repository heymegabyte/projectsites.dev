...
describe('AnalyticsGlossaryComponent', () => {
  ...
  it('renders an accessible disclosure with a definition per key metric', () => {
    ...
    expect((el.querySelector('summary') as HTMLElement).textContent).toContain('How these metrics are measured');
    ...
    for (const it of items) {
      ...
      expect((it.query(By.css('dd')).nativeElement as HTMLElement).textContent?.trim().length).toBeGreaterThan(20);
    }
  });
  ...
  it('explicitly explains requests vs page views (never conflated) and CWV honesty', () => {
    ...
    expect(text).withContext('requests≠pageviews distinction is spelled out').toContain('One page view is many requests');
