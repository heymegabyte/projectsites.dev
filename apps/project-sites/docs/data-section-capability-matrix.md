...
| **Overview summary** (table count · total records · largest table) | client-side computed from the `/data-overview` per-table `row_count` | owner | ✅ DONE (this fire) — an at-a-glance strip above the table picker: "N tables · M records · largest: <table> (K)", derived from the already-fetched row counts (no extra request). Null (hidden) until tables load so it never flashes a misleading "0 records"; `aria-live` polite. +3 Karma (derives count/total/largest · null-before-load · strip renders). Partial answer to the epic's "database metadata / table count / recent activity" Overview — D1 size/usage/limits still need the D1 REST API (blocked, no token). | honest sum of real per-table counts; no fabricated size/usage |
