...
export default defineConfig({
  ...
  testMatch: [
    ...
    'marketing-sections.spec.ts', // AL-826 — § D PLATFORM marketing homepage interactive journey: sections render (hero/social-proof/how-it-works/features/pricing/faq/footer) + FAQ accordion expand/collapse (click + keyboard) + honest AL-581 counters (never 2480) + hero-CTA scroll (SPA no-reload) + footer links + homepage axe-6bp, 0 console errors, non-mutating. Rewritten from the DELETED vanilla homepage (all 25 old tests were prod-RED; only false-green vs stale staging).
    'waiting-and-terminal.spec.ts', // AL-827 — post-create /waiting build-progress flow: no-id→home redirect · unauth real-id→GRACEFUL sign-in card (root-fixed: the auth-gated poll used to die on the first 401 → perpetual FAKE overlay; catchError keeps it alive, degrades after 2 never-loaded ticks) · sign-in→/signin?returnUrl=/waiting · go-home→homepage. 0 uncaught errors, non-mutating. Rewritten from the DELETED vanilla waiting screen (13 old tests prod-RED).
    'spa-routing.spec.ts', // AL-830 — platform SPA routing: footer legal links (privacy/terms/content) SPA-navigate (no full reload, window sentinel) + render real H1 · direct-URL routes 200 + real H1 · a bogus path returns a REAL 404 status (the soft-404 gate the old status<500 check missed). 0 console errors, non-mutating. Rewritten from the DELETED vanilla homepage (#screen-search/.footer-bottom, "Astro" — all prod-RED).
